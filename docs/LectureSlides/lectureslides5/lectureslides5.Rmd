---
title: "STAT 302, Week 5"
subtitle: "Probability, Hypothesis Testing, and Statistical Models"
author: "Bryan Martin"
date: "Week of Feb 3, 2020"
output:
...
.sectionhead[Part 1. Probability]
...
.sectionhead[Part 2. Random Variables]
...
.sectionhead[Part 3. Hypothesis Testing]
...
$1-\alpha = P(\text{don't reject} H_0 | H_0)$
]

---

.middler[
$.05 = P(\text{reject} H_0 | H_0)$

$.95 = P(\text{don't reject} H_0 | H_0)$
]
...
.middler[
$P(\text{fail to reject } H_0 | H_0 \text{ is false})$
]
...
.middler[
$\text{Power} = P(\text{reject } H_0 | H_0 \text{ is false}) = 1 - P(\text{type 2 error})$
]
...
$$T = \dfrac{\hat{\theta}-\theta_0}{\text{se}(\hat{\theta})}.$$
If our estimate is **unbiased**, that is, if 
$$\mathbb{E}[\hat{\theta}]=\theta,$$
what is $\mathbb{E}_{H_0}[T]$?

--

$0$! If our null hypothesis is true then $\theta = \theta_0$, and so the expectation of our numerator will be equal to $0$.
...
n02 <- n01 + geom_segment(x = 1.64, y = 0, xend = 1.64, yend = dnorm(1.64), 
                   colour = "blue", linetype = "dotted", lwd = 1.3) +
  scale_x_continuous(breaks = c(-3:1, 1.64, 2, 3), expand = c(0, 0),
                     labels = c("-3", "-2", "-1", "0", "1", "T", "2", "3")) + 
  theme(axis.text.x = element_text(color = c(rep("black", 5), "blue", "black", "black"))) +
  theme(axis.ticks.x = element_line(color = c(rep("black", 5), "blue", "black", "black")))
n02
```
]
...
.center[
```{r, echo = FALSE, fig.height = 4}
n03 <- n02 + stat_function(fun = dnorm, 
                xlim = c(1.64, 3),
                geom = "area",
                fill = "lightblue") + 
...
.center[
```{r, echo = FALSE, message = FALSE, fig.height = 4}
n03 + stat_function(fun = dnorm, 
                xlim = c(-1.64, -3),
                geom = "area",
                fill = "lightblue") + 
  geom_segment(x = -1.64, y = 0, xend = -1.64, yend = dnorm(-1.64), 
                   colour = "blue", linetype = "dotted", lwd = 1.3) +
  scale_x_continuous(breaks = c(-3, -2, -1.64, -1, 0 , 1, 1.64, 2, 3),
                     expand = c(0, 0),
                     labels = c("-3", "-2", "-T", "-1", "0", "1", "T", "2", "3")) + 
  theme(axis.text.x = element_text(color = c(rep("black", 2), "blue",
                                             rep("black", 3), "blue",
                                             rep("black", 2)))) +
  theme(axis.ticks.x = element_line(color = c(rep("black", 2), "blue",
                                             rep("black", 3), "blue",
                                             rep("black", 2))))
```
]
...
.sectionhead[Part 4. Linear Models]

---

# Regression

Let's assume we have some data $X_1,\ldots,X_p, Y$ where $X_1,\ldots,X_p$ are $p$ **dependent variables/explanatory variables/covariates/predictors** and $Y$ is the **independent variables/response/outcome**.
...
# $\Huge X_j$: Dependent variable

$X_j$ represents the $j$<sup>th</sup> dependent variable in our model. Notice that 
...
* $Y$: independent variable, outcome, response
* $X_j$: dependent variable, covariate, predictor
* $\beta_0$: Intercept
...
The independent variable, `Y` goes on the left-hand side of the tilde `~`, which marks the formula. 
The dependent variables are added on the right-hand side.
...
.footnote[[1] Strictly speaking, neither the independent variable nor the dependent variables are necessary to create a formula object.]
...
# <TT>summary():</TT> Regression model summaries
...
Note that by including an interaction effect, the marginal effects are automatically included!
...
Use `~ .` to include all the dependent variables in your data set. 
...
.sectionhead[Generalized Linear Models (briefly)]

---

# Generalized Linear Models

Linear models are powerful, but they are not always appropriate! For example, we typically wouldn't want to model a binary outcome using a linear model, because the error term would not make sense in the context of a binary outcome.
