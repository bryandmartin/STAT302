...
To practice this, we are going to add the `iris` data to our own packages as `my_iris` (with proper credit, of course!).

After creating `my_iris`, run `usethis::use_data(my_iris)` in your console. This will create a `data/my_iris.rda`, which contains the object `my_iris`.
...
Data is exported to the user, and so it must also be documented! The process is as follows, where you replace `my_iris` with the name of the data you are documenting:

1. Create `my_iris.R` within the `R` subdirectory.
...
.middler[<img src="iris.png" alt="" height="350"/>]

---

.center[<img src="irisdoc.png" alt="" height="550"/>]
