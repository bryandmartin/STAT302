...
Your function should return the value of $f_\lambda(x)$

**1b.** (1 point) Set $\lambda = 4$, demonstrate your function on the vector `c(-5, -3, 0, 3, 5)`. (Hint: the output should be the vector `-5, 0, 0, 0, 5`)

**1c.** (1 point) Set $\lambda = 2$, demonstrate your function on the vector `c(-5, -3, 0, 3, 5)`.
...
Write an R function that takes two parameters, numeric input `x` and a threshold `lambda`.
Your function should return the value of $g_\lambda(x)$
...
**2c.** (1 point) Set $\lambda = 2$, demonstrate your function on the vector `c(-5, -3, 0, 3, 5)`.
