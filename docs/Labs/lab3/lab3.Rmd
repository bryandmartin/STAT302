...
* Use `model.response()` to extract the model response $\mathbf{Y}$. It takes as input a model form object.
* Use `model.form()` to extract a model form object. It takes as input parameters a formula and data.
...
$$
se(\hat{\beta}_i) = \left[\sqrt{\hat{\sigma}^2(\mathbf{X}^T\mathbf{X})^{-1}}\right]_{[i,i]}
$$
Note that within the brackets of the right-hand side is a matrix and $[i,i]$ represents the $i$<sup>th</sup> row and the $i$<sup>th</sup> column index. 
* Use `diag()` to extract diagonal components from a matrix.
* `Pr(>|t|)` comes from the two-sided t test that 
$$
\begin{align}
H_0: \beta_j &= 0\\
H_a: \beta_j &\neq 0
