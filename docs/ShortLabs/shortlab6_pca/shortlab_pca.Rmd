...
**2.** Use the documentation for `prcomp()` and your intuition with data exploration to explain what the values `center` and `scale` represent.
...
ggbiplot(pc_out, groups = as.factor(penguins$species), alpha = 0.7, 
         varname.size = 3) + 
  xlim(-3, 3) +
  theme_bw()
```
